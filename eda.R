...
(d1 <- round(runif(1, 1, 6), 0))
(d2 <- round(runif(1, 1, 6), 0))
(d3 <- round(runif(1, 1, 6), 0))
(d4 <- round(runif(1, 1, 6), 0))
...
glue("You will do a total of {volume$reps} reps, spread over {volume$rounds} rounds.")
glue("Round {seq_along(1:volume$rounds)}: {sets_and_reps$sets} sets of {sets_and_reps$reps} reps, one set every {sets_and_reps$rest} seconds")
if (type != "snatch") {
